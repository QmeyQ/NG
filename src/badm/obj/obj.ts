...
export interface dressCfg {
    ...
    /**
     * 材质贴图映射，key 为材质文件名（如 "body.lmat"），值为贴图数组（不含材质位）：
     *   [0] albedoTexture | [1] occlusionTexture | [2] detailTexture | [3] detailNormal | [4] metallicGlossTexture
     * 某位为 null/undefined 则跳过该位不设置
     */
    tex?: Record<string, (string | null)[]>;
    /** 动画状态名到动画片段路径的映射 */
...
export class Obj {
    ...
    // ---------- 装扮 ----------
    /**
     * 应用换装配置：加载 mesh、材质与贴图，绑定骨骼蒙皮，加载动画片段
     * 材质路径来自 meshs 第 1..N 位，贴图来自 tex（key 为材质文件名）：
     *   [0]albedo [1]occlusion [2]detail [3]detailNormal [4]metallicGloss
     * 某位为 null 则跳过该位
     * @param cfg 换装配置
     */
    setDress(cfg: dressCfg): void {
        ...
        if (!rootBone) console.warn('[Obj] 未找到根骨骼 Hips，蒙皮将无法绑定');
            ...
            this.LC = Object.keys(cfg.meshs).length;
            this._mpm.emit("load", this.LC);
        for (const partName in cfg.meshs) {
            ...
            Res.get(meshPath, (mesh: Laya.Mesh) => {
                ...
                for (let i = 0; i < matPaths.length; i++) {
                    ...
                    if (!matPath) continue;
                    Res.get(matPath, (ma: Laya.PBRStandardMaterial) => {
                        if (!ma) return;
                        const mats = meshRenderer.materials.slice();
                        while (mats.length <= i) mats.push(null as any);
                        mats[i] = ma;
                        meshRenderer.materials = mats;
                        // 查 tex 贴图列表（key 为材质文件名），按位置设置贴图
                        const matFileName = matPath.split('/').pop();
                        const texArr = matFileName ? cfg.tex?.[matFileName] : undefined;
                        if (!texArr) return;
                        // 0 位：albedo texture
                        if (texArr[0]) Res.get(texArr[0]!, (tex) => { 
                            if (tex) {
                                ma.albedoTexture = tex;
                            } 
                        });
                        // 1 位：occlusion texture
                        if (texArr[1]) Res.get(texArr[1]!, (tex) => { if (tex) ma.occlusionTexture = tex; });
                        // 2 位：detail texture
                        if (texArr[2]) Res.get(texArr[2]!, (tex) => { if (tex) ma.detailAlbedoTexture = tex; });
                        // 3 位：detail normal
                        if (texArr[3]) Res.get(texArr[3]!, (tex) => { if (tex) ma.detailNormalTexture = tex; });
                        // 4 位：metallic gloss texture
                        if (texArr[4]) Res.get(texArr[4]!, (tex) => { if (tex) ma.metallicGlossTexture = tex; });
                        this._mpm.emit("load", --this.LC);
                    });
                }
            });
    ...
    /**
     * 注册事件监听
     * @param event 事件名，目前支持 'dressReady'
     * @param callback 回调函数，参数为 (obj: Obj)
     */
    on(event: 'load', callback: (obj: any) => void): void {
        this._mpm.on(event, callback);
    }
     ...
     */
    off(event: 'dressReady', callback?: (obj: any) => void): void {
        if (callback) {
            this._mpm.off(event, callback);
        } else {
            this._mpm.unon(event);
        }
    }
