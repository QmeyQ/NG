...
export class UI {
    ...
    /** 击球指示器精灵 */
    private _hitIndicator: Laya.Sprite | null = null;
    /** 加载进度条 */
    ...
    private _initSceneNodes(): void {
        this.root = Laya.stage.getChildByPath("root");
            ...
            this._hitIndicator = this._findNode(this.root,"Sprite") as Laya.Sprite;
    ...
    /** 设置击球指示器可见性和旋转角度 */
    setHitIndicator(visible: boolean, rotation?: number): void {
        if (this._hitIndicator) {
            this._hitIndicator.visible = visible;
            if (visible && rotation !== undefined) {
                this._hitIndicator.rotation = rotation;
            }
        }
    }
